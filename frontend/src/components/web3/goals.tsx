                    ...
                    <CardContent className="pb-3 pt-6">
                        <form onSubmit={methods.handleSubmit(onSubmit)} className="flex flex-col gap-2">
                            <div className={cn("space-y-2 w-full")}>
                                <label htmlFor="funding_goal" className="text-base">Funding Goal</label>
                                <input 
                            ...
                            {fields.map((field, index) => (
                                <div key={field.id} className={cn("space-y-2 w-full flex items-center justify-between")}
...
const inputClassName = "flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50";
const RewardTierForm: FC<{ onConfirm: (tier: RewardTier) => void, initialData?: RewardTier }> = ({ onConfirm, initialData }) => {
            ...
            <Card className="card-component">
                <CardContent className="pb-3 pt-6">
                    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-2">
                        <input {...register('title')} type="text" placeholder="Reward Tier Title" className={inputClassName} />
                        <input {...register('amount')} type="number" placeholder="Amount Required" className={inputClassName} />
                        <textarea {...register('description')} placeholder="Description" className={inputClassName} ref={descriptionRef} onChange={adjustTextareaHeight} />
